...
class VendingMachineDFA:
    ...
    def insert_money(self, amount):
        ...
        if self.balance + amount > 10000:
            print("Batas maksimal uang yang dimasukkan adalah Rp10.000!")
            return
    ...
    def buy_drink(self, choice):
        ...
        if choice in price_map and self.balance >= price_map[choice]:
            ...
            change = self.balance - price_map[choice]
            if self.refund_enabled and change > 0:
                print(f"Mengembalikan kembalian: Rp{change}")
            self.reset()
        else:
...
# Contoh penggunaan
vending_machine = VendingMachineDFA("vending_dfa.txt", refund_enabled=True)
while True:
    user_input = input("Masukkan uang atau beli minuman (1000, 2000, 5000, 10000, A, B, C, 0 untuk keluar): ")
    if user_input == "0":
